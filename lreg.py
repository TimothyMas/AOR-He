...
FILE_SPECIFIC_TIME_CUTOFFS = {
    "AORHe1_250127_163121": 96.0,
    "AORHe2_250127_164930": 118.0,
    "AORHe3_250127_170704": 120.0,
    "AORHe4_250127_172441": 170.0
}
...
def fit_piecewise_burst(t_fit, P_fit, t0):
    """
    Only consider times >= t0 for fitting.
    We do a standard burst-phase fit on (t - t0).
    """
    if len(t_fit) < 3:
    ...
    if t_prime[-1] == 0:
        ...
        Vss_init = (P_fit[-1] - Pi_init)/t_prime[-1]
...
def process_file(file_path, config):
    try:
        ...
        # Determine cutoff
        t0 = FILE_SPECIFIC_TIME_CUTOFFS.get(file_name, 0.0)
        ...
        # Plot
        plot_piecewise_model(df_all, df_fit, popt, perr, t0, r_squared,
                             file_name, config['output_plot_dir'])
